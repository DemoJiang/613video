...
import NativeVideoComponent, {
  NativeCmcdConfiguration,Commands
...
const Video = forwardRef<VideoRef, ReactVideoProps>(
  (
    ...
    const pause = useCallback(() => {
      if (!nativeRef.current) return;
      return Commands.setPlayerPauseStateCmd(nativeRef.current,true);
    }, []);

    const resume = useCallback(() => {
      if (!nativeRef.current) return;
      return Commands.setPlayerPauseStateCmd(nativeRef.current,false);
    }, []);

    const setVolume = useCallback((volume: number) => {
      if (!nativeRef.current) return;
      return Commands.setVolumeCmd(nativeRef.current, volume);
    }, []);

    const setFullScreen = useCallback((fullScreen: boolean) => {
      if (!nativeRef.current) return;
      return Commands.setFullScreenCmd(
        nativeRef.current,
        fullScreen,
      );
    }, []);
    ...
    const enterPictureInPicture = useCallback(async () => {
      ...
      const _enterPictureInPicture = () => {
        if (!nativeRef.current) return;
        Commands.enterPictureInPictureCmd(nativeRef.current);
      };
    ...
    const exitPictureInPicture = useCallback(async () => {
      ...
      const _exitPictureInPicture = () => {
        if (!nativeRef.current) return;
        Commands.exitPictureInPictureCmd(nativeRef.current);
      };
